...
class RagControllerTest {
    ...
    fun `demo sample endpoint loads reusable docs`() {
        mockMvc.perform(post("/api/rag/demo/load-sample"))
            ...
            .andExpect(jsonPath("$.suggestedQuery").value("하이브리드 검색"))
            .andExpect(jsonPath("$.docIds").isArray)
            .andExpect(jsonPath("$.docIds.length()").value(50))

        mockMvc.perform(get("/api/rag/stats").param("tenantId", "tenant-web-demo"))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.docs").value(50))
    }

    @Test
    fun `demo sample data renders a rich document graph`() {
        mockMvc.perform(post("/api/rag/demo/load-sample"))
            .andExpect(status().isOk)

        mockMvc.perform(
            get("/api/rag/admin/graph/document/tenant-web-demo/product-overview")
                .param("depth", "3")
        )
            .andExpect(status().isOk)
            .andExpect(content().string(containsString("\"relationType\":\"MENTIONS\"")))
            .andExpect(content().string(containsString("\"relationType\":\"CITES\"")))
            .andExpect(content().string(containsString("\"relationType\":\"HAS_METADATA\"")))
            .andExpect(content().string(containsString("\"label\":\"안전관제센터\"")))
            .andExpect(content().string(containsString("\"label\":\"domain=조선소\"")))
    }
    ...
    fun `demo sample clear endpoint deletes seeded docs`() {
        ...
        mockMvc.perform(post("/api/rag/demo/clear-sample"))
            ...
            .andExpect(jsonPath("$.tenantId").value("tenant-web-demo"))
            .andExpect(jsonPath("$.deletedDocs").value(50))
    ...
    @Test
    fun `plain text requests receive plain text error bodies`() {
        mockMvc.perform(
            get("/api/rag/test/plain-fail")
                .accept(MediaType.TEXT_PLAIN)
        )
            .andExpect(status().isInternalServerError)
            .andExpect(content().contentTypeCompatibleWith(MediaType.TEXT_PLAIN))
            .andExpect(content().string(containsString("INTERNAL_ERROR")))
            .andExpect(content().string(containsString("stream-like failure")))
    }
