...
`spring-boot-demo`는 `ainsoft-rag-spring-boot-starter` 사용 예제입니다. 저장 위치는 `examples/spring-boot-demo`입니다.
...
기본 설정에서는 애플리케이션 시작 시 샘플 문서 50개를 자동으로 색인합니다.
...
프론트엔드 소스는 [frontend](./frontend) 아래에 있습니다.

```bash
cd examples/spring-boot-demo/frontend
npm install
npm run dev
```
...
cd ../ainsoft-rag-spring-boot-starter/examples/spring-boot-demo/frontend
npm install
...
테스트 서버는 Lucene lock 충돌을 피하기 위해 `examples/spring-boot-demo/build/e2e-rag-index` 경로를 별도로 사용합니다.
